...
function Fig0th(files, zscales; ylbands = (-1.2, 1.1), xtickse = [-6, 0, 6], xlbands = (0, 0.2), nline = 0.65)
    fig = Figure(resolution = (0.9 * 600, 5/4 * 0.9 * 800), font = "CMU Serif Roman")
    ...
    xticks = [0]
    yticks = ([ 0, ], [ L"μ",]) 
    limits = (xlbands, ylbands)

    axB = CairoMakie.Axis(gb; limits, xticks, yticks, xlabel, ylabel, xlabelpadding = -18,  ylabelpadding = 0, yaxisposition = :right)
    ...
    dict = load(files[:c])
    xticks = ([0, 70], ["0", L"R"])
    yticks = ([last(dict["ϵs"]), 0, first(dict["ϵs"])], [L"U_{min}", L"μ", L"U_{max}"])
    ylabel = L"\epsilon"
    ...
    vlines!(ax, 0.65, ymin = 0, ymax = 0.2, color = :white, linestyle = :dash)
    text!(ax, [1.5], [0.2]; text = "●", color = :orange, align = (:center, :center))
...
## Generate Figure 
try run(`mkdir -p Figures`) catch end
p = "Data/Fig_SCM_mr0/Panel_"
files = Dict(:c => "$(p)dome.jld2", :b => "$(p)bands.jld2",  :d => "$(p)LDOS.jld2", :e => "$(p)LDOS_radial.jld2", :f => "$(p)WF_v2.jld2", :g => "$(p)minigap.jld2")
zscales = Dict(:d => 3e-2, :e => 3e-3, :g => 2e-4)
f = Fig0th(files, zscales)
save("Figures/FigSCM_pos_mr0.pdf", f)
f
